...
This action contains the shared "publish" logic used by any workflow that has already obtained
package files by some means (e.g. downloaded release assets) and just needs them pushed into the
S3-backed apt repository: check what's already published, import the GPG signing key, and run
repomanager for the rest.

See `greengage-sync-packages.yml` for an example caller: it downloads `.deb`/`.ddeb` files attached
to a GitHub release and hands the directory to this action.
...
- name: Publish downloaded packages
  uses: GreengageDB/greengage-ci/.github/actions/publish-to-s3-repo@v34
  with:
